...
/* ─────────── Quick-order matrix (recurring staples) ─────────── */
export function QuickOrderMatrix() {
  const { t, pick, money } = useLocale()
  const { qty, setQty, inc, dec, lineCount } = useWholesaleOrder()
  const rows = wholesaleProducts.filter((p) => p.recurring)

  return (
    <div className="card overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-sm p-lg border-b border-hairline">
        <div>
          <h2 className="font-serif text-card-title text-ink">{t('matrix.title')}</h2>
          <p className="font-sans text-caption text-ink-subtle mt-xxs">{t('matrix.subtitle')}</p>
        </div>
        <span className="font-sans text-caption text-ink-subtle tabular-nums rounded-pill bg-surface-2 border border-hairline px-3 py-1">{fill(t('matrix.selectedItems'), { n: lineCount })}</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse min-w-[560px]">
          <thead>
            <tr className="bg-surface-2 border-b border-hairline">
              {[t('matrix.colProduct'), t('matrix.colStock'), t('matrix.colUnit'), t('matrix.colQty'), t('matrix.colTotal')].map((h, i) => (
                <th key={i} className={cn('font-sans text-caption uppercase tracking-[0.08em] text-ink-subtle px-lg py-2.5', i >= 3 ? 'text-center' : 'text-start', i === 4 && '!text-end')}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {rows.map((p) => {
              ...
              return (
                <tr key={p.sku} className={cn('border-b border-hairline last:border-0', q > 0 && 'bg-primary/[0.03]')}>
                  <td className="px-lg py-3">
                    <div className="flex items-center gap-sm min-w-0">
                      <span className="w-10 h-10 rounded-md shrink-0" style={thumbBg(p.accent)} />
                  ...
                  <td className="px-lg py-3"><StockBadge stock={p.stock} /></td>
                  <td className="px-lg py-3">
                    <div className="font-sans text-data text-ink tabular-nums">{money(unit)}</div>
                    <div className={cn('font-sans text-caption mt-0.5', tierUp ? 'text-success' : 'text-ink-subtle')}>
                      {tierUp ? `${t('wholesale.tierActive')} ✓` : fill(t('wholesale.tierFrom'), { n: p.breakQty })}
                    </div>
                  </td>
                  <td className="px-lg py-3">
                    <div className="flex justify-center"><QtyStepper qty={q} onDec={() => dec(p.sku)} onInc={() => inc(p.sku)} onSet={(n) => setQty(p.sku, n)} size="sm" /></div>
                  </td>
                  <td className={cn('px-lg py-3 text-end font-sans text-data tabular-nums', q > 0 ? 'text-ink' : 'text-ink-subtle')}>{q > 0 ? money(unit * q, { withSymbol: false }) : '—'}</td>
                </tr>
              )
            })}
          </tbody>
...
/* Light order-summary "tear-off" — the bottom half of the merged Quick-order card. */
export function OrderSummaryFooter() {
  ...
  const checkout = () => {
    ...
    const n = commitToCart(); flash(`${n} ${t('wsum.addedToCart')}`); navigate('/checkout')
  }

  return (
    <div className="border-t border-dashed border-hairline-strong bg-surface-2 p-lg lg:p-xl">
      <div className="grid gap-lg md:grid-cols-[1fr_auto] md:items-end">
        {/* breakdown + MOQ progress */}
        <div className="flex flex-col gap-md min-w-0">
          <div>
            ...
            <div className="h-2.5 rounded-pill bg-hairline overflow-hidden">
              <span className={cn('block h-full transition-all duration-300', moqMet ? 'bg-success' : 'bg-primary')} style={{ width: `${pct}%` }} />
            </div>
            <p className={cn('font-sans text-caption mt-2', moqMet ? 'text-success' : 'text-primary-hover')}>
              {moqMet ? `✓ ${t('wsum.moqMet')}` : fill(t('wsum.moqRemaining'), { n: money(moqRemainingMinor) })}
            </p>
          </div>
        ...
        {/* amount due + CTA (trailing corner; lands inline-start in AR) */}
        <div className="flex flex-col gap-sm text-end md:min-w-[240px] md:border-s md:border-hairline md:ps-xl">
          <div className="flex items-baseline justify-between gap-md">
            <span className="font-sans text-data text-ink-muted">{t('wsum.total')}</span>
            <span className="font-serif text-headline text-primary-hover tabular-nums">{money(totalMinor)}</span>
          </div>
          <button onClick={checkout} disabled={lineCount === 0}
            className={cn('w-full inline-flex items-center justify-center gap-xs rounded-md px-4 py-3 font-sans text-button uppercase tracking-[0.06em] transition-colors',
              moqMet ? 'bg-primary text-on-primary hover:bg-primary-hover' : 'bg-hairline-strong text-ink-subtle',
              lineCount === 0 && 'opacity-50 cursor-not-allowed')}>
            <ShoppingCart size={15} /> {moqMet ? t('wsum.checkout') : t('wsum.checkoutLocked')}
          </button>
          {lineCount === 0 && <p className="font-sans text-caption text-ink-subtle text-center">{t('wsum.emptyHint')}</p>}
        </div>
      </div>
    </div>
  )
...
export function OrderRail({ variant = 'full', onReview }: { variant?: 'full' | 'compact'; onReview?: () => void }) {
  ...
  if (variant === 'compact') {
    return (
      <div className="sticky bottom-4 z-10 rounded-xl p-md text-ink-on-dark flex flex-wrap items-center justify-between gap-md shadow-soft-lg" style={{ background: 'linear-gradient(160deg,#2b2019,#17120f)' }}>
        <div className="flex items-center gap-lg flex-wrap">
          <div><p className="font-sans text-caption text-ink-on-dark-muted">{t('wsum.selected')}</p><p className="font-serif text-card-title text-ink-on-dark tabular-nums">{lineCount}</p></div>
          <span className="w-px h-8 bg-hairline-dark" />
          <div><p className="font-sans text-caption text-ink-on-dark-muted">{t('wsum.subtotal')}</p><p className="font-serif text-card-title text-primary-bright tabular-nums">{money(subtotalMinor)}</p></div>
          <div className="max-w-[220px]">
            <div className="h-1.5 w-40 rounded-pill bg-hairline-dark overflow-hidden"><span className="block h-full bg-primary" style={{ width: `${pct}%` }} /></div>
            <p className={cn('font-sans text-caption mt-1', moqMet ? 'text-success' : 'text-primary-bright')}>{moqMet ? `✓ ${t('wsum.moqMet')}` : fill(t('wsum.moqRemaining'), { n: money(moqRemainingMinor) })}</p>
          </div>
        </div>
        <button onClick={() => onReview?.()} disabled={lineCount === 0} className={cn('inline-flex items-center gap-xs rounded-md px-4 py-2.5 font-sans text-button uppercase tracking-[0.06em] transition-colors', lineCount > 0 ? 'bg-primary text-on-primary hover:bg-primary-hover' : 'bg-hairline-dark text-ink-on-dark-muted cursor-not-allowed')}>
          {t('wsum.review')} <ArrowRight size={15} className="rtl:rotate-180" />
        </button>
      </div>
    )
  }
