...
import { CustomerProvider, useCustomer, type RedeemOption } from '@/state/CustomerContext'
import { customer, tierOrder, type CustomerOrder, type LoyaltyTier, type SavedAddress } from '@/data/account'
...
const statusVariant: Record<CustomerOrder['status'], 'gold' | 'success' | 'neutral'> = {
...
function OverviewPanel({ onTab }: { onTab: (id: string) => void }) {
  ...
  const { points, tier, lifetimeSpendMinor } = useCustomer()
    ...
    <div className="flex flex-col gap-lg">
        ...
        <div className="relative flex flex-col gap-md">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="font-serif text-display-md text-ink-on-dark capitalize">{t(`loyalty.tier.${tier}`)}</p>
            </div>
            <Sparkles className="text-primary-bright" size={28} />
          </div>
          <div className="flex items-end gap-xs">
            <span className="font-serif text-display-md text-primary-bright tabular-nums">{points.toLocaleString()}</span>
          ...
          <button onClick={() => onTab('loyalty')} className={buttonClass('primary', 'sm', 'self-start mt-xs')}>{t('loyalty.redeem')}</button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-sm">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => onTab('orders')} className="link-gold">{t('cta.viewAll')} <ChevronRight size={15} className="rtl:rotate-180" /></button>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-sm">
        {([
          ...
          <button key={a.id} onClick={() => onTab(a.id)} className="card card-hover p-lg flex flex-col items-start gap-sm">
            <span className="grid place-items-center w-10 h-10 rounded-md bg-primary/10 text-primary-hover"><a.icon size={18} /></span>
...
const PERKS: Record<LoyaltyTier, { en: string; ar: string }[]> = {
  taster: [{ en: 'Earn 1 point per SAR', ar: 'نقطة لكل ريال' }, { en: 'Free shipping over SAR 200', ar: 'شحن مجاني فوق ٢٠٠ ر.س' }],
  connoisseur: [{ en: 'Earn 1.25 points per SAR', ar: '١٫٢٥ نقطة لكل ريال' }, { en: 'Early access to limited art-cards', ar: 'وصول مبكر للبطاقات المحدودة' }],
  maison: [{ en: 'Earn 1.5 points per SAR', ar: '١٫٥ نقطة لكل ريال' }, { en: 'Personal concierge & a birthday box', ar: 'كونسيرج شخصي وعلبة ميلاد' }],
}

function LoyaltyPanel() {
  const { t, money, pick } = useLocale()
  const { points, tier, lifetimeSpendMinor, pointsHistory } = useCustomer()
  ...
  return (
    <div className="flex flex-col gap-lg">
      <div className="card p-lg flex flex-wrap items-center justify-between gap-md">
        ...
        <button onClick={() => setRedeemOpen(true)} className={buttonClass('primary', 'sm')}>{t('loyalty.redeem')}</button>
      </div>
      ...
      <RewardsModal open={redeemOpen} onClose={() => setRedeemOpen(false)} />
    </div>
  )
}
...
function RewardsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { t } = useLocale()
  const { points, redeem } = useCustomer()
  const [done, setDone] = useState<string | null>(null)

  const rewards: { opt: RedeemOption; icon: typeof Ticket }[] = [
    { opt: { id: 'shipping', label: { en: t('redeem.shipping'), ar: t('redeem.shipping') }, cost: 300, kind: 'shipping' }, icon: Snowflake },
    { opt: { id: 'v25', label: { en: t('redeem.voucher25'), ar: t('redeem.voucher25') }, cost: 500, kind: 'voucher' }, icon: Ticket },
    { opt: { id: 'v75', label: { en: t('redeem.voucher75'), ar: t('redeem.voucher75') }, cost: 1200, kind: 'voucher' }, icon: Ticket },
    { opt: { id: 'artcard', label: { en: t('redeem.artcard'), ar: t('redeem.artcard') }, cost: 1500, kind: 'artcard' }, icon: Sparkles },
  ]

  const onRedeem = (r: RedeemOption) => {
    if (redeem(r)) {
      setDone(r.id)
  ...
  return (
    <Modal open={open} onClose={onClose} eyebrow={t('account.tab.loyalty')} title={t('redeem.title')} size="md">
      <div className="flex items-center justify-between rounded-lg bg-canvas-dark text-ink-on-dark p-md mb-md">
        <span className="font-sans text-caption uppercase tracking-[0.12em] text-primary-bright">{t('redeem.balance')}</span>
        <span className="font-serif text-headline text-primary-bright tabular-nums">{points.toLocaleString()} <span className="text-card-title text-ink-on-dark-muted">{t('redeem.cost')}</span></span>
      </div>
      <div className="flex flex-col gap-sm">
        {rewards.map(({ opt, icon: Icon }) => {
          const can = points >= opt.cost
          const isDone = done === opt.id
          return (
            <div key={opt.id} className={cn('flex items-center gap-md rounded-lg border p-md', isDone ? 'border-success/40 bg-success/8' : 'border-hairline')}>
              <span className="grid place-items-center w-10 h-10 rounded-md bg-primary/10 text-primary-hover shrink-0"><Icon size={18} /></span>
              <div className="flex-1 min-w-0">
                <p className="font-sans text-data text-ink">{opt.label.en}</p>
                <p className="font-sans text-caption text-ink-subtle tabular-nums">{opt.cost} {t('redeem.cost')}</p>
              </div>
              {isDone ? (
                <span className="inline-flex items-center gap-xs font-sans text-caption text-success"><Check size={14} /> {t('cta.added')}</span>
              ) : (
                <button onClick={() => onRedeem(opt)} disabled={!can} className={buttonClass('primary', 'sm')}>
                  {can ? t('redeem.redeem') : t('redeem.notEnough')}
                </button>
              )}
            </div>
          )
...
function SubscriptionsPanel() {
  ...
  if (subscriptions.length === 0) return <EmptyState icon={Repeat} title={t('subs.empty')} body="" cta={{ to: '/shop', label: t('subs.startCta') }} />
        ...
        return (
          <div key={s.id} className="card overflow-hidden flex flex-col sm:flex-row">
            <div className="sm:w-40 relative shrink-0" style={{ backgroundColor: tint(f.accent, 14) }}>
              <div className="aspect-square sm:h-full">{found && <ProductArt flavorId={found.product.flavorId} kind="box" />}</div>
            </div>
            <div className="flex-1 p-lg flex flex-col gap-sm">
              <div className="flex items-start justify-between gap-sm">
                <div>
                  <h3 className="font-serif text-card-title text-ink">{pick(s.title)}</h3>
                  <p className="font-sans text-caption text-ink-subtle">{s.cadence === 'monthly' ? t('subs.everyMonth') : t('subs.everyQuarter')} · {money(s.priceMinor)}</p>
                </div>
                <StatusBadge variant={s.status === 'active' ? 'success' : 'neutral'}>{t(`subs.status.${s.status}`)}</StatusBadge>
              </div>
              <p className="font-sans text-data text-ink-muted">{t('subs.next')}: {new Date(s.nextRenewal).toLocaleDateString(locale === 'ar' ? 'ar-SA' : 'en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
              <div className="flex items-center gap-xs mt-auto pt-xs">
                <button onClick={() => toggleSubscription(s.id)} className={buttonClass('secondary', 'sm')}>{s.status === 'active' ? t('subs.pause') : t('subs.resume')}</button>
                <button onClick={() => setConfirmCancel(s.id)} className={buttonClass('ghost', 'sm')}>{t('subs.cancel')}</button>
              </div>
            </div>
          </div>
        )
    ...
    <div className="flex flex-col gap-lg">
      <div className="grid sm:grid-cols-2 gap-md">
        {giftCards.map((g) => {
          ...
          return (
            <div key={g.code} className="relative rounded-xl overflow-hidden p-lg text-ink-on-dark animate-scale-in" style={{ backgroundColor: '#3b241a' }}>
              <div className="absolute inset-0 opacity-30"><WaveDivider tone="gold" height={20} /></div>
              <div className="relative flex flex-col gap-md">
                <div className="flex items-center justify-between">
                ...
                <div className="h-1.5 rounded-pill bg-surface-dark-1 overflow-hidden"><span className="block h-full bg-primary" style={{ width: `${pct}%` }} /></div>
      ...
      <div className="flex flex-wrap gap-sm">
        <button onClick={() => setBuyOpen(true)} className={buttonClass('primary')}><CreditCard size={16} /> {t('giftcard.buy')}</button>
        <button onClick={() => setRedeemOpen(true)} className={buttonClass('secondary')}><Ticket size={16} /> {t('giftcard.redeem')}</button>
      </div>

      <BuyGiftCardModal open={buyOpen} onClose={() => setBuyOpen(false)} />
      <RedeemGiftCardModal open={redeemOpen} onClose={() => setRedeemOpen(false)} />
    </div>
  )
}

function BuyGiftCardModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { t, money } = useLocale()
  const { buyGiftCard } = useCustomer()
  const amounts = [10000, 20000, 30000, 50000]
  const [amount, setAmount] = useState(20000)
  const [email, setEmail] = useState('')
  const [done, setDone] = useState(false)

  const purchase = () => { buyGiftCard(amount, email); setDone(true); setTimeout(() => { setDone(false); onClose() }, 1400) }

  return (
    <Modal open={open} onClose={onClose} eyebrow={t('account.tab.giftcards')} title={t('gift.buyTitle')} size="md"
      footer={done ? undefined : <><button onClick={onClose} className={buttonClass('ghost', 'sm')}>{t('common.cancel')}</button><button onClick={purchase} className={buttonClass('primary', 'sm')}>{t('gift.purchase')} · {money(amount)}</button></>}>
      {done ? (
        <div className="py-lg flex flex-col items-center gap-sm text-center">
          <span className="grid place-items-center w-14 h-14 rounded-pill bg-success/12 text-success"><Check size={26} /></span>
          <p className="font-serif text-card-title text-ink">{t('gift.purchased')}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-md">
          {/* preview */}
          <div className="relative rounded-xl overflow-hidden p-lg text-ink-on-dark" style={{ backgroundColor: '#3b241a' }}>
            <div className="absolute inset-0 opacity-30"><WaveDivider tone="gold" height={18} /></div>
            <div className="relative flex items-center justify-between">
              <div>
                <p className="eyebrow text-primary-bright">{t('giftcard.balance')}</p>
                <p className="font-serif text-display-md text-ink-on-dark tabular-nums">{money(amount)}</p>
              </div>
              <Gift size={28} className="text-primary-bright" />
            </div>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-xs">
              {amounts.map((a) => (
                <button key={a} onClick={() => setAmount(a)} className={cn('px-4 py-2 rounded-md border font-sans text-data tabular-nums transition-all', amount === a ? 'border-primary bg-primary/10 text-ink' : 'border-hairline-strong text-ink-muted hover:border-ink/30')}>{money(a)}</button>
              ))}
            </div>
          </div>
          <label className="flex flex-col gap-xs">
            <span className="label">{t('gift.recipientEmail')}</span>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="friend@example.com" className="input" />
          </label>
        </div>
      )}
    </Modal>
...
function RedeemGiftCardModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { t, money } = useLocale()
  const { redeemGiftCard } = useCustomer()
  const [code, setCode] = useState('')
  const [result, setResult] = useState<{ ok: boolean; minor: number } | null>(null)

  const submit = () => {
    const r = redeemGiftCard(code)
    setResult(r)
    if (r.ok) setTimeout(() => { setResult(null); setCode(''); onClose() }, 1600)
  }

  return (
    <Modal open={open} onClose={onClose} eyebrow={t('account.tab.giftcards')} title={t('gift.redeemTitle')} size="sm"
      footer={<><button onClick={onClose} className={buttonClass('ghost', 'sm')}>{t('common.cancel')}</button><button onClick={submit} disabled={!code.trim()} className={buttonClass('primary', 'sm')}>{t('giftcard.redeem')}</button></>}>
      <div className="flex flex-col gap-sm">
        <label className="flex flex-col gap-xs">
          <span className="label">{t('gift.enterCode')}</span>
          <input value={code} onChange={(e) => { setCode(e.target.value); setResult(null) }} placeholder="JAZ-GIFT-XXXX" className="input tracking-[0.1em] uppercase" />
        </label>
        {result && (result.ok
          ? <p className="inline-flex items-center gap-xs font-sans text-data text-success"><Check size={15} /> {t('gift.added')} · {money(result.minor)}</p>
          : <p className="font-sans text-data text-danger">{t('gift.invalidCode')}</p>)}
      </div>
    </Modal>
  )
}
...
function AddressesPanel() {
  const { t, pick } = useLocale()
  const { addresses, makeDefaultAddress } = useCustomer()
...
function PrivacyPanel() {
  const { t, pick } = useLocale()
  const { consents, notifications, setConsent, setNotif, ...rest } = useCustomer()
  const [eraseOpen, setEraseOpen] = useState(false)
  const [erased, setErased] = useState(false)

  const exportData = () => {
    const doc = {
      profile: { name: rest.name.en, tier: rest.tier, points: rest.points },
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="card overflow-hidden">
      <div className="px-lg py-md bg-surface-2 border-b border-hairline"><h3 className="font-serif text-card-title text-ink">{title}</h3></div>
...
/* ───────────── shared ───────────── */
function ConfirmDialog({ open, onClose, title, body, confirmLabel, onConfirm, danger, eyebrow }: {
